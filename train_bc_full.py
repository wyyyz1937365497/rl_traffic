            ...
            for veh_type in ['main', 'ramp', 'diverge']:
                ...
                if logits_key in info and info[logits_key] is not None:
                    ...
                    if valid_len > 0:
                        ...
                        # 收集统计信息
                        pred_action = pred_logits[:valid_len].argmax(dim=-1).float() / 10.0
                        target_action = target_idx[:valid_len].float() / 10.0
                        all_preds.extend(pred_action.detach().cpu().numpy())
                        all_targets.extend(target_action.detach().cpu().numpy())
