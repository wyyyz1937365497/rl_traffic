...
class SUMOCompetitionFramework:
    ...
    def apply_control_algorithm(self, step):
        """
        初赛优化策略：OCR 最大化 (GLOSA + 防死锁)
        核心原则：
        1. 尽量不停车通过 J15 (绿灯通过)
        2. 防止 J15 下游拥堵导致路口锁死
        3. 忽略稳定性惩罚 (根据规则 W_stability=0)
        4. 忽略干预成本惩罚 (根据规则 P_int=1)
        """
        
        # ==================== 配置参数 ====================
        # 关键瓶颈路口
        BOTTLENECK_TL = 'J15' 
        # 瓶颈上游路段 (E10 主路，E17 匝道)
        UPSTREAM_EDGES = ['E10', 'E17'] 
        # 瓶颈下游路段 (E11，用于检测拥堵)
        DOWNSTREAM_EDGE = 'E11'
        # 拥堵阈值 (下游路段等待车辆数)
        CONGESTION_THRESHOLD = 10 
        # ICV 车辆类型标识 (需要根据 routes.xml 确认，通常是 'cv', 'auto', 'icv' 等)
        # 建议先打印 traci.vehicle.getTypeID(veh_id) 确认哪些是可控车辆
        ICV_TYPES = ['cv', 'auto', 'icv', 'type1'] # 请根据实际情况修改
        ...
        try:
            if BOTTLENECK_TL in self.available_traffic_lights:
                tl_state = traci.trafficlight.getRedYellowGreenState(BOTTLENECK_TL)
                next_switch_time = traci.trafficlight.getNextSwitch(BOTTLENECK_TL)
        except:
            pass
        
        # 判断当前是否是绿灯 (简单判断，具体需根据 state 字符串，'G'或'g'表示绿灯)
        is_green = 'G' in tl_state or 'g' in tl_state
        
        # ==================== 获取下游拥堵状态 ====================
        is_congested = False
        try:
            # 检查下游路段等待车辆数
            waiting_vehicles = traci.edge.getLastStepWaitingTime(DOWNSTREAM_EDGE)
            # 或者检查车辆数
            vehicle_count = traci.edge.getLastStepVehicleNumber(DOWNSTREAM_EDGE)
        ...
        for veh_id in vehicle_ids:
            try:
                ...
                # 如果类型不在列表中，跳过 (避免控制社会车辆导致作弊嫌疑)
                # 如果比赛允许控制所有车，可注释掉此判断
                is_icv = False
                for icv_t in ICV_TYPES:
                    if icv_t.lower() in v_type.lower():
                        is_icv = True
                        break
                if not is_icv:
                    continue
                ...
                if edge_id in UPSTREAM_EDGES:
                    ...
                    dist_to_end = edge_length - lane_pos
                    ...
                    if dist_to_end < 500:
                        ...
                        if is_congested:
                            # 下游拥堵，强制减速，防止涌入路口
                            # 即使当前是绿灯也不让走，避免堵在路口中间
                            target_speed = max(5.0, current_speed - 2.0)
                        
                        # --- 策略 B: GLOSA 绿灯通过 (优先级次之) ---
                        elif not is_green:
                            # 当前是红灯，计算到达路口时间
                            time_to_junction = dist_to_end / max(current_speed, 1.0)
                            estimated_arrival_time = current_time + time_to_junction
                            
                            # 如果预计在下次变灯前能到达，且到达时是红灯，则减速等待
                            # 如果预计到达时刚好变绿，则保持或加速
                            # 简单逻辑：如果是红灯且距离很近，减速准备停车；如果距离远，调整速度争取绿灯
                            if estimated_arrival_time < next_switch_time:
                                # 会在当前相位内到达
                                if not is_green:
                                    # 红灯期间到达，减速避免急刹
                                ...
                                if current_speed < max_speed * 0.8:
                                    target_speed = min(max_speed, current_speed + 1.0)
                        
                        # --- 策略 C: 自由流恢复 ---
                        else:
                            # 绿灯且无拥堵，恢复最大速度
                            if current_speed < max_speed - 2.0:
                                target_speed = min(max_speed, current_speed + 1.5)
